...
async function validateEarlyAdopters() {
    ...
    const examplePagesPath = !isCI ? '../example-pages.js' : 'example-pages.js';
    const registryPath = !isCI ? '../registry.yaml' : 'registry.yaml';


    const earlyAdoptersFileContent = fs.readFileSync(examplePagesPath, 'utf8');
    ...
    for (const user of users) {
        try {
            ...
            if (!response.headers['content-type'].startsWith('image/')) {
                // TODO: also check that the image is from trusted sources (e.g. GitHub)?
